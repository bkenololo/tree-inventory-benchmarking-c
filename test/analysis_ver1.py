...
C_FILE = "ver1_test.c"
EXEC_FILE = "./run_ll"
OUTPUT_CSV = "metrics_ll.csv"
DATA_SIZES = {'Small': 1000, 'Medium': 50000, 'Large': 200000}

# Ukuran memori 64-bit Mac: Tree = 64 bytes, Pointer = 8 bytes. Total Node = 72 bytes.
NODE_SIZE_BYTES = 72 

def compile_c():
    print(f"[*] Compiling {C_FILE}...")
    # Pakai gcc/clang bawaan macOS
...
def run_analysis():
    ...
    for label, n in DATA_SIZES.items():
        print(f"[*] Analyzing Linked List for {label} size (N={n})...")
        
        # Eksekusi program C dan tangkap outputnya
        ...
        # Kalkulasi Metrics
        memory_mb = (n * NODE_SIZE_BYTES) / (1024 * 1024)
        
        # Throughput (Operasi per detik). Handle pembagian dengan 0 kalau terlalu cepat.
...
def save_to_csv(data):
    ...
    print(f"[+] Linked List analysis complete! Saved to {OUTPUT_CSV}")
